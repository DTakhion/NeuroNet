...
internal constructor(
    ...
    private suspend fun beginProxyLoop(
        ...
        // [MODIFICACION 2] Recibimos upstream
        ...
        try {
            ...
            factory
                .create(
                    ...
                    socketCreator = socketCreator,
                    serverDispatcher = serverDispatcher,
                    // [MODIFICACION 3] Pasamos upstream a la Fábrica del Manager
                    upstream = upstream
                )
                .loop(
                    lock = lock,
    ...
    private suspend fun proxyLoop(
        ...
        socketCreator: SocketCreator,
        serverDispatcher: ServerDispatcher,
        // [MODIFICACION 4] Recibimos upstream
        upstream: SocketFactory,
    ) {
        ...
        if (isHttpEnabled) {
            scope.launch(context = Dispatchers.Default) {
                beginProxyLoop(
                    ...
                    socketCreator = socketCreator,
                    serverDispatcher = serverDispatcher,
                    // [MODIFICACION 5] Pasamos upstream
                    upstream = upstream,
                )
            }
        }
        ...
        if (isSocksEnabled) {
            scope.launch(context = Dispatchers.Default) {
                beginProxyLoop(
                    ...
                    socketCreator = socketCreator,
                    serverDispatcher = serverDispatcher,
                    // [MODIFICACION 6] Pasamos upstream
                    upstream = upstream,
                )
            }
        }
    ...
    private suspend fun startServer(
        ...
        socketCreator: SocketCreator,
        serverDispatcher: ServerDispatcher,
        // [MODIFICACION 7] Recibimos upstream
        upstream: SocketFactory,
    ) {
        try {
            ...
            coroutineScope {
                ...
                launch(context = Dispatchers.Default) {
                    proxyLoop(
                        ...
                        socketCreator = socketCreator,
                        serverDispatcher = serverDispatcher,
                        // [MODIFICACION 8] Pasamos upstream al loop
                        upstream = upstream,
                    )
                }
            }
    ...
    // [MODIFICACION 9] Implementamos la nueva firma de SharedProxy
    override suspend fun start(
        lock: Locker.Lock,
        connectionStatus: Flow<BroadcastNetworkStatus.ConnectionInfo>,
        upstream: SocketFactory, // <--- El parámetro nuevo
    ) =
                                    ...
                                    proxyJob =
                                        launch(context = Dispatchers.Default) {
                                            startServer(
                                                ...
                                                socketCreator = socketCreator,
                                                serverDispatcher = serverDispatcher,
                                                // [MODIFICACION 10] Inyectamos el upstream en el inicio del servidor
                                                upstream = upstream,
                                            )
                                        }
                                }
    ...
    private data class ProxyState(val http: Boolean, val socks: Boolean) {
        ...
        @CheckResult
        fun isReady(preferences: ProxyPreferences): Boolean {
            // Nota: Eliminé las corrutinas aquí para simplificar, asumiendo que las prefs
            // se leen de forma síncrona o ya tienes los valores.
            // Si preferences.listen... requiere ser suspendido, mantenlo como estaba.
            return true
        }
    }
